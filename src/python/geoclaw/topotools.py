...
class Topography(object):
    ...
    def read_header(self):
        ...
        if abs(self.topo_type) in [2,3]:
            ...
            self._x = np.linspace(x_origin,
                               x_origin + (z.shape[0] - 1) * dx, z.shape[0])
            self._y = np.linspace(y_origin - (z.shape[0] - 1) * dy,
                               y_origin, z.shape[1])
    ...
    def crop(self, filter_region=None, coarsen=1, buffer=0, align=None):
        ...
        if (coarsen > 1) and (align is not None):
            xs = np.array([self.x[ilower + i] for i in range(coarsen)])
            ...
            ys = np.array([self.y[jlower + j] for j in range(coarsen)])
        ...
        iupper = np.minimum(len(self.x)-1, iupper + buffer*coarsen) + 1
        jupper = np.minimum(len(self.y)-1, jupper + buffer*coarsen) + 1
    ...
    def make_function(self, interp_kwargs={}):
        ...
        if 'fill_value' not in interp_kwargs.keys():
            interp_kwargs['fill_value'] = np.nan
...
def read_netcdf(path, zvar=None, extent='all', coarsen=1, return_topo=True,
    ...
    # buffer, checking limits of arrays:
    i1 = np.maximum(0, ilower - buffer*coarsen)
    j1 = np.maximum(0, jlower - buffer*coarsen)
    i2 = np.minimum(len(x)-1, iupper + buffer*coarsen) + 1
    j2 = np.minimum(len(y)-1, jupper + buffer*coarsen) + 1
